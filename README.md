...
As you can see, the Route attribute has defined the public route for this endpoint (.../person/1/order/recent), while the method name (GetRecentOrders) is more developer-friendly. An external consumer would issue a GET request to http://yourservice/person/1/order/recent, and this method would handle that call. Note that the method could have been called simply "RecentOrders", or "Recent", or even just "Get" since there are no other methods on this controller so far. The name of the method does not *have* to match its route in any way.
